...
setStreamState _ Stream{streamState} val = do
    print ("set-state", val)
    writeIORef streamState val
...
closed ctx@Context{concurrency,streamTable} strm@Stream{streamState,streamNumber} cc = do
    print ("closing", streamNumber)
